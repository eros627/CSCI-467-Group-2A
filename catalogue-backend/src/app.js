...
import { shippingQuoteRoutes, shippingRateRoutes } from './routes/shipping-routes.js';
...
export function createApp({ config, services }) {
    ...
    app.use('/api/parts', catalogRoutes(services.catalogService));
    app.use('/api/shipping-quote', shippingQuoteRoutes(services.adminService));
    app.use('/api/orders', orderRoutes({
        orderService: services.orderService,
        adminService: services.adminService,
        warehouseService: services.warehouseService,
        adminApiKey: config.auth.adminApiKey,
        warehouseApiKey: config.auth.warehouseApiKey,
    }));
    app.use(
        '/api/inventory',
        requireApiKey(config.auth.warehouseApiKey, 'Warehouse'),
        inventoryRoutes(services.warehouseService),
    );
    app.use(
        '/api/shipping-rates',
        requireApiKey(config.auth.adminApiKey, 'Administrator'),
        shippingRateRoutes(services.adminService),
    );
    app.use(
        '/api/warehouse',
        requireApiKey(config.auth.warehouseApiKey, 'Warehouse'),
