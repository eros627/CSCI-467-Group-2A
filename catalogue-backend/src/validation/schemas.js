...
export const partSearchQuery = paginationQueryShape().extend({
    q: z.string().trim().max(100).default(''),
}).transform(({ q, ...pagination }) => ({ ...pagination, search: q || undefined }));
...
export const customerOrderQuery = z.object({ email: z.string().trim().email().max(254).optional() });
...
export const paginationQuery = paginationQueryShape();
...
export const shippingRateBody = z.object({
    ...
    (value) => value.maxWeight === null || value.maxWeight > value.minWeight,
    { message: 'maxWeight must be greater than minWeight', path: ['maxWeight'] },
);

export const shippingRateIdParams = z.object({
    rateId: z.coerce.number().int().positive(),
});

export const shippingQuoteQuery = z.object({
    weight: z.coerce.number().finite().nonnegative().max(99999999),
});

const orderStatus = z.string().trim().transform((value) => value.toUpperCase()).pipe(
    z.enum(['PENDING_PAYMENT', 'AUTHORIZED', 'PAYMENT_FAILED', 'SHIPPED']),
);
...
export const orderSearchQuery = paginationQuery.extend({
    ...
    if (value.dateFrom && value.startDate && value.dateFrom !== value.startDate) {
        context.addIssue({ code: z.ZodIssueCode.custom, message: 'Use either dateFrom or startDate', path: ['startDate'] });
    }
    if (value.dateTo && value.endDate && value.dateTo !== value.endDate) {
        context.addIssue({ code: z.ZodIssueCode.custom, message: 'Use either dateTo or endDate', path: ['endDate'] });
    }
}).transform(({ startDate, endDate, ...value }) => ({
    ...value,
    dateFrom: value.dateFrom || startDate,
    dateTo: value.dateTo || endDate,
}));
