...
- **Application database (PostgreSQL):** inventory, shipping brackets, orders, item snapshots, status history, and inventory movements. The SQL contract is in [`database/schema.sql`](database/schema.sql).
...
Prerequisites: Node.js 20+, pnpm/npm, PostgreSQL 16+ for application data, and access to the legacy MySQL catalog.

```bash
cp .env.example .env
pnpm install
psql -U auto_parts -d auto_parts_app -f database/schema.sql
psql -U auto_parts -d auto_parts_app -f database/seed-development.sql   # optional local data
pnpm dev
```
...
For a local application database, `docker compose up -d postgres` creates PostgreSQL and applies both SQL files. Point these `.env` values at it:
...
|---|---|---|---|
| `GET` | `/api/parts` (alias: `/api/products`) | Paginate catalog with available quantity | Public |
| `GET` | `/api/parts/search?q=...` | Search catalog | Public |
...
| `GET` | `/api/shipping-quote?weight=...` | Quote shipping for a total weight | Public |
| `GET` | `/api/orders?startDate=&endDate=&status=&minPrice=&maxPrice=` | Search orders | Admin or warehouse key |
| `GET` | `/api/orders/:id` | Staff order detail; `?email=...` retains customer lookup | Staff key or customer email match |
| `PATCH` | `/api/orders/:id` | Ship an authorized order with carrier/tracking body | Warehouse key |
| `POST` | `/api/orders/:id/payment` | Retry a failed payment | Customer email match |
| `POST` | `/api/inventory/receive` | Receive stock by part number or exact description | Warehouse key |
| `GET/POST` | `/api/shipping-rates` | List/create shipping brackets | Admin key |
| `PUT/DELETE` | `/api/shipping-rates/:rateId` | Update/delete one shipping bracket | Admin key |
...
4. Inventory rows are locked inside a PostgreSQL transaction, stock is reserved, and the pending order/item snapshots are saved atomically.
...
- Preserve PostgreSQL `NUMERIC` precision; repository mappers convert returned numeric strings to JavaScript numbers at the API boundary.
- Keep `orders.idempotency_key` and `orders.payment_transaction_id` unique.
- Do not add a foreign key from PostgreSQL inventory/items to the legacy MySQL database; cross-database foreign keys are not available and deployments may use different servers.
- Checkout relies on PostgreSQL transactions and `SELECT ... FOR UPDATE`.
...
Tests mock the repositories and credit-card service; they do not contact PostgreSQL, MySQL, or NIU systems. Readiness at `/health/ready` is the intentional runtime exception: it checks both configured database connections.
